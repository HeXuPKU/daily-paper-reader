...
tldr: 本研究开发了HealthFormer，一个基于解码器架构的生成式多模态生理模型。通过对1.5万余人的深度表型数据进行训练，该模型能够整合血液、微生物组、可穿戴设备等七大领域的667项指标，预测个体健康轨迹。HealthFormer在疾病风险预测和干预模拟（如个性化营养）方面表现卓越，为构建临床“数字孪生”提供了基础。
source: arxiv
...
motivation: 旨在解决理解人类健康随时间变化以及个体对干预措施反应差异这一医学核心挑战。
method: 采用仅解码器的Transformer架构，将跨越七个领域的667项生理测量指标Token化，通过生成式目标训练模型预测个体健康轨迹。
result: 模型在30项疾病和死亡终点预测中优于临床风险评分，并能准确模拟临床试验中的干预效果，预测方向与实际一致。
conclusion: HealthFormer作为一种初始的“健康世界模型”，为疾病风险分层和干预模拟提供了强有力的生成式框架。
---

## 摘要
理解人类健康随时间的变化，以及个体对干预反应差异的原因，仍然是医学领域的一个核心挑战。在此，我们提出了 HealthFormer，这是一种仅解码器（decoder-only）的 Transformer 模型，通过在“人类表型项目”（Human Phenotype Project）的数据上进行训练，以生成式方式对人类生理轨迹进行建模。该项目是一个包含超过 15,000 名深度表型化个体的多次随访队列。我们将每位参与者的健康轨迹在跨越七个领域的 667 项测量指标中进行标记化（tokenise）：血液生物标志物、身体成分、睡眠生理学、持续血糖监测、肠道微生物组、可穿戴设备衍生的生理学，以及行为和药物暴露。我们训练 HealthFormer 来预测这些领域的个体生理轨迹，通过这一单一的生成目标，一系列临床相关任务可以表达为对模型的查询。我们展示了在没有特定任务训练的情况下，HealthFormer 可以迁移到四个独立队列，并改进了 30 个发病和死亡终点中 27 个的预测，在所有比较中均超过了既有的临床风险评分。我们进一步证明该模型可以进行计算机模拟（in silico）干预：在留出的个性化营养试验中，以干预为条件的预测恢复了个体六个月的生物标志物变化（例如，舒张压的 Pearson r = 0.78）。在从已发表试验中提取的 41 项随机干预-结果比较中，我们的结果显示预测的效应方向在所有情况下均一致，且预测均值在 30 个案例中落入报告的 95% 置信区间内。我们将 HealthFormer 定位为初步的健康世界模型，预测、风险分层和干预条件模拟均可作为查询从中产生，为临床数字孪生提供了基础。
