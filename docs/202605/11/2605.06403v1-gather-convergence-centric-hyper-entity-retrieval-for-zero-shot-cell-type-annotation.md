...
tldr: 针对零样本单细胞类型注释中多基因查询（超实体）导致的检索效率低和LLM成本高的问题，本文提出GATHER。该方法通过全局多源图遍历识别拓扑收敛点，捕捉基因间的协同效应，无需LLM参与检索过程即可提取高质量证据。在多个生物数据集上，GATHER仅需单次LLM调用便实现了超越现有KG-RAG基准的准确率，显著提升了检索效率和性能。
source: arxiv
...
motivation: 现有的知识图谱检索增强生成方法在处理包含数百个基因的超实体查询时，因依赖局部实体推理而面临扩展性差和LLM成本高昂的挑战。
method: 提出一种以收敛为中心的检索器，通过全局图遍历寻找多个输入基因共同指向的拓扑收敛点，并结合节点与路径重要性评分筛选证据。
result: "在免疫和肺部数据集上，GATHER以单次LLM调用的极低成本实现了最高27.45%和59.64%的精确匹配准确率，优于ToG和RoG等强基准。"
conclusion: 拓扑收敛点能有效压缩多实体信号，为超实体查询提供了一种比局部推理更高效、信息量更大的全局检索替代方案。
---
