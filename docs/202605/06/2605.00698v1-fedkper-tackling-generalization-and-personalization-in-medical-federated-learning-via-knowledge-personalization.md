...
tags: ["query:med-ai"]
score: 6.0
evidence: 医疗机构的医疗联邦学习
tldr: 针对医疗联邦学习中由于数据异构性导致的泛化与个性化失衡及遗忘问题，本文提出FedKPer框架。该框架在本地训练阶段引入知识个性化，通过选择性对齐缓解异构性影响，并在全局聚合时优先考虑可靠且标签多样化的更新。实验证明，FedKPer在不牺牲知识保留的前提下，显著优化了模型的泛化与个性化平衡，提升了医疗诊断的准确性。
source: arxiv
...
motivation: 医疗机构间的数据统计异构性使得全局模型难以同时实现良好的泛化能力和本地适应性，并导致了严重的遗忘问题。
method: 引入FedKPer框架，通过本地知识个性化训练和基于可靠性及标签多样性的全局聚合机制来平衡泛化与个性化。
result: 实验表明FedKPer在提升模型泛化与个性化性能的同时，有效缓解了模型在更新过程中的知识遗忘。
conclusion: 通过选择性对齐和改进的聚合策略，FedKPer成功解决了医疗联邦学习中异构性带来的挑战，实现了更优的性能权衡。
---

## 摘要
联邦学习（FL）在医疗应用中具有巨大潜力。然而，医疗机构之间的统计异质性给联邦学习带来了重大挑战，因为全局模型既难以在未见过的患者群体中进行泛化，也难以适应各个医院独特的本地数据分布。这种异质性还加剧了全局和局部层面的遗忘问题，导致模型更新后，先前学习到的患者模式被错误分类。尽管先前的工作大多将泛化和个性化视为独立的挑战，但我们证明，通过与全局模型的选择性对齐以及改进的聚合方案，可以在两者之间实现更好的平衡，从而共同减轻统计异质性的影响。具体而言，我们提出了 FedKPer，它在每个本地设备的训练阶段引入了知识个性化。随后，通过全局模型聚合过程来考虑泛化性，其中强调了可靠且标签多样化的本地更新。我们评估了 FedKPer 的性能，并设计了与遗忘常见后果相关的额外指标。总的来说，我们证明了 FedKPer 在不牺牲记忆保留能力的情况下，改善了泛化与个性化之间的权衡。
