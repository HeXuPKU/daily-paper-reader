...
tags: ["query:med-ai"]
score: 7.0
evidence: 整合分子、形态和临床元数据的三模态对比学习
tldr: "本研究提出Haiku，一个连接空间生物学与临床组织学的三模态对比学习模型。该模型整合了多重免疫荧光（mIF）、H&E染色图像及临床元数据，基于11种器官的2670万个空间蛋白质组学补丁进行训练。Haiku实现了跨模态检索、零样本生物标志物推断及生存预测，显著优于单模态基准。此外，其反事实预测框架能揭示与癌症进展相关的分子变化，为整合空间生物学与临床背景提供了强大的分析工具。"
source: arxiv
...
motivation: 旨在解决目前缺乏系统性框架来联合建模分子、形态和临床数据以进行生物医学研究的局限性。
method: "开发了名为Haiku的三模态对比学习模型，在共享嵌入空间中对齐mIF、H&E图像和临床元数据。"
result: Haiku在跨模态检索、生存预测（C-index 0.737）和零样本生物标志物推断方面均表现出色，显著超越现有方法。
conclusion: 该研究证明了三模态对齐能有效整合空间生物学与临床背景，为生物学探索和临床预测提供了新途径。
---

## 摘要
整合分子、形态学和临床数据对于基础和转化生物医学研究至关重要，但联合建模这些模态的系统框架仍然有限。在这里，我们提出了 Haiku，这是一个在多重免疫荧光（mIF）上训练的三模态对比学习模型。它包含来自 11 种器官类型、1,606 名患者的 3,218 个组织切片的 2,670 万个空间蛋白质组学图块，并将匹配的苏木精-伊红（H&E）组织学和临床元数据对齐在一个共享的嵌入空间中。Haiku 实现了三向跨模态检索，在下游分类和临床预测任务上优于单模态基准，并支持通过仅以临床元数据文本描述为条件的融合检索进行零样本生物标志物推断。在各项任务中，Haiku 的表现均优于竞争方法，实现了跨模态检索（Recall@50 高达 0.611，而基准接近于零）、生存预测（C-index 0.737，相对提升 +7.91%）以及零样本生物标志物推断（52 种生物标志物的平均 Pearson 相关系数为 0.718）。此外，我们引入了一个反事实预测框架，在该框架中，仅修改临床元数据而固定组织形态，可以揭示与乳腺癌分期进展和肺癌生存结果相关的特定生态位（niche-specific）分子转变。在一项肺腺癌案例研究中，反事实分析恢复了以 CD8 和颗粒酶 B 增加、PD-L1 减少和 Ki67 降低为特征的特定生态位转变，这与报道的良好预后模式大致一致。我们将这些反事实结果作为探索性的、产生假设的信号，而非机制性断言。这些能力表明，通过 Haiku 进行的三模态对齐实现了空间生物学的综合分析，为生物学探索架起了分子测量与临床背景之间的桥梁。
