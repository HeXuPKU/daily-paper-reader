...
tags: ["query:med-ai"]
score: 7.0
evidence: 临床元数据与分子数据的整合用于临床预测
tldr: "本研究提出Haiku，一个三模态对比学习模型，旨在整合空间蛋白质组学（mIF）、组织病理学（H&E）和临床元数据。通过在涵盖11种器官、1606名患者的大规模数据集上训练，Haiku实现了跨模态检索、零样本生物标志物推断及生存预测。该模型不仅在多项任务中优于单模态基准，还引入了反事实预测框架，为探索组织形态与临床结果之间的分子关联提供了新工具，有效桥接了空间生物学与临床组织学。"
source: arxiv
...
motivation: 现有的生物医学研究缺乏系统性框架来联合建模分子、形态和临床这三种关键模态的数据。
method: "开发了名为Haiku的三模态对比学习模型，在共享嵌入空间中对齐多重免疫荧光、H&E染色图像和临床元数据。"
result: Haiku在跨模态检索、生存预测（C-index达0.737）和零样本生物标志物推断方面均显著优于现有方法。
conclusion: Haiku通过三模态对齐实现了空间生物学与临床背景的深度整合，为生物医学探索和临床预测提供了强大的分析工具。
---
