...
date: 2026-05-11
pdf: "https://www.biorxiv.org/content/10.64898/2026.04.22.720142v4.full.pdf"
tags: ["query:gwas"]
score: 7.0
evidence: 基于泛基因组的变异调用流程，用于改进已知和新变异的检测
tldr: 针对现有泛基因组变异检测工具难以识别新发变异的局限，本研究开发了PanVariants流程。该流程通过整合k-mer与比对方法，实现了对已知及新发变异的高精度检测。在GIAB、临床及群体队列样本的系统评估中，PanVariants在SNV、Indel及SV检测性能上均显著优于BWA+GATK和DRAGEN等传统及现有泛基因组方案，为基因组学研究从线性参考向泛基因组参考转变提供了最佳实践框架。
source: biorxiv
selection_source: fresh_fetch
motivation: 现有的泛基因组变异检测工具大多局限于识别图谱中已有的已知变异，无法满足对新发变异准确发现的需求。
method: 开发了集成k-mer与比对方法的PanVariants流程，并结合针对DNBSEQ数据微调的DeepVariant模型来提升检测精度。
result: "PanVariants在SNV检测中比BWA+GATK减少了73%的错误，在SV检测中F1分数达到89.39%，性能接近长读长测序水平。"
conclusion: PanVariants为泛基因组变异检测建立了高效、准确的标准流程，有力推动了泛基因组参考基因组在临床诊断和群体遗传学中的应用。
---

## 摘要
背景：尽管与线性参考基因组相比，泛基因组参考提供了更丰富的群体多样性，但目前主流的基于泛基因组的变异检测工具仅限于检测存储在图中的已知变异。为了解决这一局限性，我们开发了 PanVariants，这是一种旨在提高已知和新发变异检测准确性的新型流程。我们在三种情境下系统地评估了其性能，并与传统的线性比对方案（BWA+GATK/Manta）以及现有的泛基因组感知方案（DRAGEN/PanGenie）进行了对比：包括 Genome in a Bottle 样本中小变异（SNVs/indels）和结构变异（SVs）的准确性、临床阳性样本的检测，以及在队列联合分型中的应用。结果：通过整合基于 k-mer 和基于比对的方法，PanVariants 显著减少了变异误差（假阳性 + 假阴性），在 SNVs 检测中，相比 BWA+GATK 减少了 73%，相比 DRAGEN 减少了 45%。使用高质量的 DNBSEQ 数据重新训练 DeepVariant 模型使误差进一步降低了 15%。在 SVs 检测方面，PanVariants 的 F1 分数达到 89.39%，显著优于 DRAGEN (68.18%) 和 BWA+Manta (58.33%)，接近长读长测序的性能 (95.22%)。在临床阳性样本的验证中，PanVariants 成功检测到了所有预期的致病变异，而 PanGenie 则未能检出。在队列联合分型分析中，PanVariants 比 GATK 检测到更多变异，孟德尔遗传错误更少，且单样本准确性更高。结论：PanVariants 为基于泛基因组的变异检测建立了一个稳健的框架和最佳实践流程，实现了对新发变异的敏感发现以及对 SNVs、indels 和 SVs 的高准确度检测。我们对可选处理步骤和输入变量的系统评估为用户提供了实用指导。经过诊断和群体应用验证，我们的研究结果有力地支持了未来基因组学从线性参考基因组向泛基因组参考基因组的转变。
