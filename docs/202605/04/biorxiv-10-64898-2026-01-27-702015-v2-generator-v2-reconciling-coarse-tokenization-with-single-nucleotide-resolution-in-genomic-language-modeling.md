...
tags: ["query:med-ai"]
score: 8.0
evidence: 用于序列理解和概率推理的基因组基础模型
tldr: 本研究针对基因组基础模型在长序列建模中难以平衡长程上下文、单核苷酸分辨率与计算效率的问题，提出了 GENERator-v2。通过引入分解核苷酸监督（FNS）和基因组压缩预训练（GCP），该模型在保持高效 k-mer 分词的同时实现了单核苷酸级别的预测精度。实验表明，该模型在支持高达 98k bp 上下文的情况下，在多项基准测试中达到或超越了现有最先进水平，显著提升了推理效率。
source: biorxiv
selection_source: fresh_fetch
motivation: 现有的基因组模型在处理长序列时，难以在维持单核苷酸分辨率的同时兼顾计算效率和对稀疏功能信号的捕捉。
method: 提出了 FNS 技术以协调 k-mer 分词与单核苷酸似然，并结合 GCP 策略优化训练数据分布，使其聚焦于基因核心及调控区域。
result: 该系列自回归模型支持高达 98k bp 的上下文，在零样本评估和下游微调任务中均优于或匹配现有 SOTA 模型，且推理更高效。
conclusion: 通过将监督机制和数据分布与基因组的生物学结构对齐，为构建可扩展且生物学保真的基因组语言模型提供了一条有效路径。
---

## 摘要
基因组基础模型旨在直接从 DNA 序列中学习通用表示，从而在广泛的生物学任务中实现序列理解、生成和概率推理。然而，由于长程上下文、核苷酸级分辨率与实际计算效率之间的矛盾，将此类模型扩展到基因组长度仍然具有挑战性。架构创新虽然实现了越来越长的名义输入，但往往难以将额外的上下文转化为显著的性能提升，特别是在真核生物基因组中存在稀疏功能信号的情况下。在这项工作中，我们从训练目标和数据构建的角度重新审视了长上下文基因组基础模型的设计。我们引入了因子化核苷酸监督（Factorized Nucleotide Supervision, FNS），通过概率边缘化将高效的 k-mer 分词与单核苷酸似然相调和；以及基因组压缩预训练（Genome Compression Pretraining, GCP），通过专注于以基因为中心和调控区域来重塑训练分布。这些技术共同使标准的基于 Transformer 的模型能够在不牺牲核苷酸级保真度或计算效率的情况下，执行功能性上下文学习。基于这些想法，我们提出了一系列自回归基因组基础模型，支持真核和原核基因组中高达 98k 碱基对的上下文。在免训练评估和下游微调基准测试中，我们的模型始终优于先前的方法，在实现更高效推理的同时，达到或超过了最先进的基准水平。总之，这些结果表明，将监督和数据方案与基因组序列的生物结构相对齐，为实现可扩展且生物学上忠实的基因组语言建模提供了一条有原则且有效的路径。用于下游分析的模型、数据和脚本可在 https://huggingface.co/GenerTeam 公开获取。
