...
tags: ["query:med-ai"]
score: 6.0
evidence: 电子健康记录预训练表示学习用于药物重定位
tldr: 电子健康记录和医保数据中的表示学习已用于疾病预测，但难以支撑科学假设发现，现有BERT模型也常忽略医学编码层级与诊疗交互。作者提出统一预训练框架，融合层级子词聚合、部分掩码和交叉引用机制。模型在预训练目标及痴呆、住院等下游预测上持续领先，并在阿尔茨海默病药物重定位中数据驱动重现已知候选药。进一步引入任务自适应表示控制，缓解历史处方过度编码，形成假设生成与优先排序的探索性筛选流程。
source: arxiv
selection_source: fresh_fetch
motivation: 电子健康记录表示学习多用于疾病预测，却难以发现科学假设；现有BERT模型忽略医学编码层级与诊疗交互。
method: 提出统一预训练框架，融合层级子词聚合、部分掩码与交叉引用机制；并引入任务自适应表示控制以缓解历史处方过度编码。
result: 在预训练目标和痴呆发病、住院等下游预测上持续优于现有方法；阿尔茨海默病药物重定位案例中，数据驱动重现已知候选药并稳健排序。
conclusion: 该框架建立从假设生成到优先排序的探索性筛选流程，不提供因果证据，但可为后续严格因果推断筛选有希望的候选。
---

## 摘要
从电子健康记录和医疗索赔数据中的医疗编码序列进行表征学习，已在多种临床应用（如疾病预测）中取得成功。然而，将这一方法拓展至科学假说的发现仍面临重大挑战。一个原因是，许多现有的基于BERT的模型未能充分捕捉医疗编码的层次结构以及诊断与治疗之间的复杂交互。为解决这些局限，我们提出了一种新的统一预训练框架，该框架显式整合了层次化子词元聚合、部分掩码和交叉引用机制。所提出的模型在预训练目标以及下游临床事件预测任务（包括痴呆发病和住院）上均持续优于现有方法。我们还开展了一项针对阿尔茨海默病的计算机模拟药物重定位案例研究。在假说生成步骤中，我们的方法以数据驱动的方式成功重新发现了已知的有前景药物，而无需依赖文献等外部知识来源。随后，在假说优先级排序步骤中，我们引入了一种任务自适应表征方法，以缓解诊断向量中历史处方信息的过度编码，从而能够对生成的假说进行稳健的优先级排序。本研究建立了一个基于观察性关联的探索性筛选工作流，用于假说生成和优先级排序。重要的是，该框架并非旨在提供因果证据，而是为后续严格的因果推断识别有前景的候选物。总体而言，本研究表明，领域知识指导的表征学习与任务自适应表征控制相结合，能够实现一种实用的假说发现工作流。
