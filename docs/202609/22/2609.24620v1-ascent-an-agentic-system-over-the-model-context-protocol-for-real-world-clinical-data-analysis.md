...
title_zh: Ascent：一个基于模型上下文协议的智能体系统，用于真实世界临床数据分析
authors: "Angelo Ziletti, Leonardo D'Ambrosi, Melanie Tuchardt, Tim Kondziella"
date: 2026-09-21
pdf: "https://arxiv.org/pdf/2609.24620v1"
tags: ["query:med-ai"]
score: 7.0
evidence: 基于模型上下文协议的临床真实世界数据与队列分析智能体系统
tldr: 从真实世界临床数据回答流行病学问题，需医学编码、模式感知SQL，并校验人群、分母和时间等隐含选择。Ascent 通过模型上下文协议（MCP）提供统一工具面，覆盖医学编码、问答与队列分析，兼容标准化和原生模式。作者提出 EpiTrap 检验系统能否避开已知药物流行病学错误，并比较固定流程与不同模型/编排器下的智能体。能力强的模型下，智能体在原生与标准化模式上分别平均提升 27 和 20 个百分点，但需更多工具调用和更长运行时间，体现真实项目价值。
source: arxiv
selection_source: fresh_fetch
figures_json: "[{\"url\": \"assets/figures/arxiv/2609.24620v1/fig-001.webp\", \"caption\": \"\", \"page\": 3, \"index\": 1, \"width\": 1942, \"height\": 2507}]"
motivation: 真实世界临床数据分析需处理医学编码、SQL 与人群、分母、时间等隐含假设，易产生药物流行病学错误。
method: Ascent 基于模型上下文协议（MCP）统一暴露医学编码、问答和队列分析工具，兼容标准化与原生模式，并用 EpiTrap 测试系统避错能力。
result: 在能力强的模型上，智能体比固定流程在原生和标准化模式上平均准确率分别提升 27 和 20 个百分点，代价是更多工具调用和更长运行时间。
conclusion: 真实项目经验表明，该系统对可行性评估、诊断迭代和专家引导分析有价值。
---

## 摘要
从真实世界临床数据中回答流行病学问题，需要医学编码、模式感知的 SQL，以及对关于人群、分母和时间的隐含选择进行验证。我们提出 Ascent，一个智能体系统，它通过共享的模型上下文协议工具界面，为标准化模式和原生模式提供医学编码、问答和队列分析。我们引入 EpiTrap，一个用于测试系统能否避免已知药物流行病学错误的数据集，并在不同模型和编排器下比较固定流水线与智能体。在使用能力较强的模型时，智能体在原生模式和标准化模式上分别将准确率较固定流水线平均提高 27 和 20 个百分点。这些提升需要更多的工具调用和更长的运行时间。来自实际项目的经验凸显了该系统在可行性评估、诊断迭代和专家引导分析方面的价值。
