...
motivation: 针对临床结局预测任务，探讨直接监督学习是否比通用的自监督预训练具有更强的归纳偏置。
method: 提出一种通过最大化类间分离度与类内方差之比来显式塑造表示几何结构的监督深度学习框架。
result: 在死亡率和再入院等预测任务中，该方法在相同模型容量下全面超越了掩码、自回归和对比预训练基准。
conclusion: 在低熵且结局驱动的医疗领域，直接监督是比大规模自监督预训练更优的表示学习驱动方式。
---
