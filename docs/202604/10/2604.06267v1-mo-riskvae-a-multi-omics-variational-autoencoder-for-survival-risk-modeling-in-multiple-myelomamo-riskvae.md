...
tldr: 本研究针对多发性骨髓瘤的生存风险建模，探讨了多模态变分自编码器（VAE）在整合多组学数据时的潜在空间设计问题。研究发现，生存监督下的模型性能主要受潜在正则化强度和结构的影响，而非具体的散度公式。基于此，作者提出了MO-RiskVAE，通过适度放宽KL正则化并引入Gumbel-Softmax混合潜在空间结构，优化了风险梯度的对齐。该模型在不增加额外监督的情况下，显著提升了生存风险分层的准确性和稳定性。
source: arxiv
...
motivation: 传统的VAE在生存风险建模中由于潜在空间正则化过于严格，往往无法有效保留与预后相关的关键变异信息。
method: 提出MO-RiskVAE模型，通过系统优化潜在正则化尺度并引入基于Gumbel-Softmax的连续-离散混合潜在空间结构来增强风险建模。
result: 实验表明，适度放宽KL正则化并结合混合潜在空间设计，能显著改善模型对生存风险的判别能力和全局排序。
conclusion: MO-RiskVAE通过优化潜在空间建模策略，在不增加复杂启发式方法的情况下，有效提升了多发性骨髓瘤的生存风险分层性能。
---
