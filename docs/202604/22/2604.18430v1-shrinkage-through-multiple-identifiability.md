...
tldr: 本文提出了一个经验贝叶斯框架，用于聚合针对同一因果参数的多个识别泛函估计量。该方法通过后验均值池化一组渐近线性估计量，在精确识别和存在均值零偏差的两种情形下均能保持一致性。研究引入了潜在异质性超参数来指导推断，并提供了频率派置信区间和贝叶斯预测区间，为结合随机对照试验与观测证据提供了有效的统计工具。
source: arxiv
...
motivation: 旨在解决当同一因果参数有多个识别路径时，如何有效整合这些可能带有偏差的估计量以提升估计性能。
method: 采用经验贝叶斯框架，通过后验均值池化多个估计量，并利用工作独立性假设处理样本间的依赖关系。
result: 在精确识别和偏差均值为零且泛函数量增长的两种非嵌套机制下，均确立了估计量的一致性。
conclusion: 该框架通过潜在异质性参数提供了互补的推断手段，能够灵活应对从精确一致到存在异质性的多种实际场景。
---

## 摘要
我们提出了一个经验贝叶斯框架，用于聚合从与同一因果参数相关的多个识别泛函中获得的估计量。核心对象是一个后验均值，它汇集了一组关于标量因果目标的渐近线性估计量。我们在两种非嵌套情形下建立了相合性：一是精确可识别性，即每个泛函都识别相同的因果效应；二是第二种情形，即单个泛函存在偏差，但各泛函之间的识别偏差均值为零，且泛函数量随样本量增加。通过在同一样本上评估所有估计量所产生的依赖性，是通过一种保持点估计量相合性的工作独立性机制来处理的。推断过程围绕一个潜在的异质性超参数展开：当该参数消失时，各泛函共享一个共同目标，我们通过夹心方差或子抽样报告该目标的频率派置信区间；当该参数严格大于零时，每个泛函的目标都是从混合分布中真实抽取的样本，我们为新泛函的潜在目标构建渐近有效的贝叶斯预测区间。这两种推断输出基于不同的假设集，因此是互补而非排他的。我们在利用观测证据增强随机对照试验的背景下阐述了该框架。
