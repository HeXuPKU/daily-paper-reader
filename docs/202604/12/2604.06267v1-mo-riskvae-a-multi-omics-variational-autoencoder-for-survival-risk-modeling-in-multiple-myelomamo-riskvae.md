...
motivation: 标准VAE的潜在正则化策略在生存监督下容易限制预后相关信息的表达，导致多组学集成模型的风险预测性能不稳定。
method: 通过系统控制变量研究正则化规模与结构的影响，并引入基于Gumbel-Softmax的连续-离散混合潜在空间构建MO-RiskVAE。
result: 适度放宽KL正则化显著提升了生存判别力，且混合潜在空间结构有效增强了模型对生存风险梯度的全局排序能力。
conclusion: MO-RiskVAE在不引入复杂启发式方法的情况下，通过优化潜在空间设计实现了比原始MyeVAE更稳健且更优的风险分层性能。
---
