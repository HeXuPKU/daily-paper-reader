...
title_zh: FairMed-XGB：一种用于关键医疗数据中人口统计学公平性的贝叶斯优化多指标可解释框架
authors: "Mitul Goswami, Romit Chatterjee, Arif Ahmed Sekh"
date: 2026-03-16
pdf: "https://arxiv.org/pdf/2603.14947v1"
tags: ["query:med-ai"]
score: 8.0
evidence: 使用MIMIC-IV和eICU数据库研究重症医疗数据的公平性
tldr: 针对重症监护中机器学习模型的性别偏见问题，本文提出FairMed-XGB框架。该框架通过贝叶斯优化将多种公平性指标整合进XGBoost的损失函数中，在保持高预测准确率的同时显著降低了人口统计学偏见。通过在MIMIC-IV-ED和eICU数据集上的验证，证明了其在减少偏见和提高模型透明度方面的有效性，为医疗AI的公平部署提供了可靠方案。
selection_source: fresh_fetch
motivation: 解决重症监护临床决策中机器学习模型存在的性别偏见问题，以增强临床信任和治疗公平性。
method: 提出一种集成公平性感知损失函数（结合统计差异、泰尔指数和瓦瑟斯坦距离）并利用贝叶斯搜索优化的XGBoost框架。
result: "在多个临床队列中显著降低了各项偏见指标（如SPD下降达51%），且预测准确率（AUC-ROC）的损失微乎其微。"
conclusion: FairMed-XGB为高风险医疗环境提供了一种鲁棒、可解释且符合伦理要求的公平决策解决方案。
---

## 摘要
部署在重症监护环境中的机器学习模型表现出人口统计学偏见，特别是性别差异，这损害了临床信任和公平治疗。本文介绍了 FairMed-XGB，这是一种新型框架，能够在保持模型性能和透明度的同时，系统地检测并减轻基于性别的预测偏见。该框架集成了一个公平感知损失函数，结合了统计差异（Statistical Parity Difference）、泰尔指数（Theil Index）和 Wasserstein 距离，并通过贝叶斯搜索在 XGBoost 分类器中进行联合优化。对源自 MIMIC-IV-ED 和 eICU 数据库的七个临床不同队列进行的缓解后评估表明，偏见显著减少：MIMIC-IV-ED 上的统计差异降低了 40% 至 51%，eICU 上降低了 10% 至 19%；泰尔指数下降了四个到五个数量级，接近于零；Wasserstein 距离减少了 20% 至 72%。这些收益是在预测准确性几乎没有下降（AUC-ROC 下降 <0.02）的情况下实现的。基于 SHAP 的可解释性揭示了该框架减少了对性别代理特征的依赖，为临床医生提供了关于偏见如何以及在何处得到纠正的可操作见解。FairMed-XGB 为公平的临床决策提供了一种稳健、可解释且符合伦理的解决方案，为在重症医疗环境中部署值得信赖的人工智能铺平了道路。
