...
tags: ["query:med-ai"]
score: 10.0
evidence: 真实世界场景驱动的医疗大语言模型评估基准
tldr: 针对大语言模型在医学考试中表现优异但在真实医疗场景中表现不佳的问题，本文提出了QuarkMedBench。这是一个基于真实场景的基准测试，包含超过2.4万条单轮和多轮对话。研究团队开发了一套结合多模型共识与证据检索的自动评分框架，生成了细粒度的评分细则。实验证明该框架与专家评估高度一致，能更准确地衡量模型在复杂医疗问题上的性能，并支持动态知识更新。
selection_source: fresh_fetch
motivation: 现有的医学评估多依赖选择题，无法捕捉真实医疗咨询中非结构化、模糊且长尾的复杂性。
method: 构建了涵盖临床、健康和专业咨询的大规模数据集，并提出一种结合证据检索与多模型共识的动态细粒度自动评分框架。
result: "自动评分细则与临床专家的一致率达91.8%，并揭示了主流模型在处理真实临床细节时与传统考试指标的显著性能差异。"
conclusion: QuarkMedBench为评估LLM处理复杂健康问题的能力提供了严谨、可重复且支持动态更新的基准工具。
---
