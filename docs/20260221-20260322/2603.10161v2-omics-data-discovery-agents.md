...
tags: ["query:med-ai"]
score: 9.0
evidence: 用于获取和转换组学数据的大模型智能体
tldr: 针对生物医学文献中组学数据难以重用的问题，本文提出了一种基于大语言模型（LLM）的智能体框架。该框架能够自动抓取文献、提取元数据、识别并下载数据集，并利用容器化工具执行量化分析。研究证明了该系统在数据集识别和结果复现方面的有效性，并展示了其在跨研究比较中的潜力，为实现大规模自动化数据重用提供了新途径。
selection_source: fresh_fetch
motivation: 生物医学文献中海量的组学数据因存储分散且缺乏结构化信息，导致其在计算重用和结果复现方面面临巨大挑战。
method: 开发了一个基于LLM的智能体框架，通过模型上下文协议（MCP）调用容器化分析工具，实现从文献检索到数据重分析的全自动化流程。
result: "该系统实现了80%的数据集识别准确率，重分析结果与原报告高度吻合，并成功识别出肝纤维化研究中一致的蛋白质调控模式。"
conclusion: 该工作建立了一个将静态文献转化为可执行、可查询资源的框架，为大规模自动化组学数据重用奠定了基础。
---
