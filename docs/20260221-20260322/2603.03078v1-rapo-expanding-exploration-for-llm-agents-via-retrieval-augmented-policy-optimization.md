...
tags: ["query:med-ai"]
score: 8.0
evidence: 用于大模型智能体的智能体强化学习
tldr: 针对大语言模型智能体在强化学习中探索受限的问题，本文提出检索增强策略优化（RAPO）框架。该框架通过引入外部检索机制，在训练中结合离策步骤级轨迹进行混合策略采样，并利用检索感知优化机制校准策略梯度。实验证明，RAPO在多项任务上显著提升了性能和训练效率，为智能体探索提供了新思路。
selection_source: fresh_fetch
motivation: 现有智能体强化学习过度依赖同策探索，限制了对新推理视角的发现，且缺乏细粒度的步骤级探索机制。
method: 提出两阶段框架，通过混合策略展开让智能体学习检索到的外部步骤级轨迹，并利用检索感知优化校准策略梯度。
result: "在14个数据集上实现了5.0%的平均性能提升，并将训练效率提高了1.2倍。"
conclusion: RAPO通过显式引入检索机制有效扩展了智能体的探索空间，是提升大语言模型智能体推理能力的有效且高效的方法。
---
