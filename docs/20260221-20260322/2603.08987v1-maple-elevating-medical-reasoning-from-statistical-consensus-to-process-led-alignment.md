...
tags: ["query:med-ai"]
score: 10.0
evidence: 将医疗过程奖励模型与测试时强化学习结合以增强推理
tldr: 针对医疗大模型在复杂推理中依赖多数投票（MV）导致可靠性不足的问题，本文提出MAPLE框架。该框架将医疗过程奖励模型（Med-RPM）与测试时强化学习（TTRL）相结合，通过专家对齐的细粒度监督取代传统的统计共识。实验证明，MAPLE能有效将搜索智能转化为模型参数记忆，在多个医疗基准测试中显著提升了推理的准确性与可靠性。
selection_source: fresh_fetch
motivation: 传统的测试时强化学习依赖多数投票作为监督信号，在复杂医疗场景下难以保证临床正确性。
method: 提出MAPLE框架，利用医疗过程奖励模型（Med-RPM）提供细粒度的专家对齐监督，引导强化学习过程。
result: 在四个医疗基准测试上的实验结果显示，该方法显著优于现有的TTRL方法和独立的PRM筛选策略。
conclusion: 从随机启发式统计转向结构化的步骤奖励，是开发可靠且可扩展的医疗人工智能系统的关键。
---

## 摘要
近期，医疗大语言模型的进展探索了测试时强化学习（TTRL）以增强推理能力。然而，标准的 TTRL 通常依赖多数投票（MV）作为启发式监督信号，这在复杂的医疗场景中可能并不可靠，因为最频繁的推理路径并不一定是临床正确的。在这项工作中，我们提出了一种新颖且统一的训练范式，将医疗过程奖励模型与 TTRL 相结合，以弥合测试时缩放（TTS）与参数化模型优化之间的差距。具体而言，我们通过使用 Med-RPM 将传统的 MV 替换为细粒度、专家对齐的监督范式，从而改进了 TTRL 框架。这种集成确保了强化学习是由医疗正确性而非仅仅是共识所引导的，从而有效地将基于搜索的智能蒸馏到模型的参数记忆中。在四个不同基准测试上的广泛评估表明，我们开发的方法始终且显著地优于当前的 TTRL 和独立的 PRM 选择。我们的研究结果表明，从随机启发式方法转向结构化的分步奖励，对于开发可靠且可扩展的医疗人工智能系统至关重要。
