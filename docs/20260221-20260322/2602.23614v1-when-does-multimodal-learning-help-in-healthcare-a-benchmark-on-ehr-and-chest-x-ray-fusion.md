...
tags: ["query:med-ai"]
score: 9.0
evidence: 涉及电子健康记录(EHR)融合的多模态学习基准
...
motivation: 旨在明确多模态学习在医疗实践中（特别是存在模态缺失和公平性约束时）何时以及为何能真正提升临床决策支持。
method: 在标准化的MIMIC-IV和MIMIC-CXR队列上，对多种EHR与CXR融合策略、鲁棒性及公平性进行了系统性基准评估。
result: 多模态融合在信息互补时提升性能，但受EHR时间结构导致的模态不平衡影响，且在模态缺失时性能迅速下降，且未能自动改善公平性。
conclusion: 提供了关于医疗多模态学习适用场景的实践指导，并发布了开源工具包CareBench以促进可部署、可靠的多模态系统开发。
---
