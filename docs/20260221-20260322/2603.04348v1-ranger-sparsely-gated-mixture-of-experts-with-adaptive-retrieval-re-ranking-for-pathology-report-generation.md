...
tags: ["query:med-ai"]
score: 8.0
evidence: 用于病理报告生成的混合专家模型框架与检索技术
...
motivation: 现有的病理报告生成方法受限于同质化解码架构和静态知识检索，难以处理复杂的形态差异且易引入外部噪声。
method: 提出 RANGER 框架，在解码器中集成稀疏门控 MoE 以实现动态诊断模式特化，并利用自适应检索重排序模块优化知识库信息的语义对齐。
result: 在 PathText-BRCA 数据集上，RANGER 在 BLEU-4 (0.1435)、METEOR (0.1883) 和 ROUGE-L (0.3038) 等多项指标上均取得了最优性能。
