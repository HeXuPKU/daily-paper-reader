...
tags: ["query:med-ai"]
score: 8.0
evidence: 大模型协作中的多智能体强化学习
tldr: 针对大语言模型（LLM）多智能体协作中稀疏终端奖励导致的信用分配难题，本文提出上下文反事实信用分配（C3）方法。该方法通过冻结对话上下文并评估替代方案，隔离单个消息的因果影响，从而提取无偏且低方差的边际优势。在数学和编程基准测试中，C3 显著提升了协作性能，并增强了信用分配的准确性与智能体间的因果依赖。
selection_source: fresh_fetch
motivation: 多智能体强化学习中稀疏的终端反馈导致决策层面的信用分配困难，难以识别单个智能体行为对最终结果的贡献。
method: 引入 C3 方法，通过冻结上下文、评估替代方案的固定延续回放以及应用留一法（LOO）基准来隔离消息的因果影响。
result: 在五个数学和编程基准测试中，C3 显著优于现有基准，并展现出更高的信用保真度和更强的智能体间因果依赖。
conclusion: C3 通过局部干预有效解决了轨迹层面的信用扩散问题，为提升 LLM 多智能体协作性能提供了稳健的强化学习优化方案。
---
