...
result: "在三个医疗VQA基准测试中，Med-Evo显著提升了基础模型的性能，其中在SLAKE数据集上实现了10.43%的准确率增长。"
conclusion: Med-Evo证明了医疗多模态大模型可以通过无标签测试数据的自进化实现性能跨越，为缓解医疗数据标注难题提供了新路径。
---

## 摘要
医疗多模态大语言模型（MLLMs）在各种医疗保健任务中展现出了卓越的能力。然而，目前的后训练策略（如监督微调和强化学习）严重依赖大量的标注数据，而忽视了无标注测试数据在模型增强方面的潜力。这种局限性在医疗领域尤为突出，因为由于严格的数据敏感性和标注复杂性，获取大规模标注医疗数据非常困难。此外，利用测试数据在从无标注样本中生成可靠的监督信号以及维持稳定的自我进化方面也面临挑战。为了解决这些局限性，我们提出了 Med-Evo，这是首个针对医疗 MLLMs 的自我进化框架，它利用无标签强化学习在不需要额外标注数据的情况下提升模型性能。我们的框架引入了两项关键创新：1) 特征驱动伪标签（FPL），它从所有异构候选响应中识别语义质心，以便在每次采样过程中选择伪标签；2) 硬-软奖励（HSR），它结合了精确匹配、标记级评估和语义相似度，以提供分层奖励。在三个医疗 VQA 基准测试和两个基础 MLLMs 上的实验表明，我们的方法相较于现有最先进（SOTA）方法具有明显优势，在使用 Qwen2.5-VL 的 SLAKE 数据集上，准确率显著提高了 10.43%，召回率提高了 4.68%，证明了我们方法的有效性。
