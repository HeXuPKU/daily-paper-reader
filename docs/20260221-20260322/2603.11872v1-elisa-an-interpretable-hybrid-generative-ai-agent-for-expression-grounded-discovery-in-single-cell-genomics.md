...
tags: ["query:med-ai"]
score: 10.0
evidence: 结合scGPT与大语言模型的单细胞基因组学AI智能体
tldr: ELISA是一个可解释的混合生成式AI智能体，旨在解决单细胞转录组数据分析中AI缺乏直接表达谱访问及模型不透明的问题。它结合了scGPT嵌入、BioBERT语义检索和LLM推理，支持基因特征和自然语言查询。通过集成通路评分、配体-受体预测等模块，ELISA在细胞类型检索和生物学发现复现方面显著优于现有工具，为单细胞基因组学研究提供了从数据探索到假设生成的桥梁。
selection_source: fresh_fetch
motivation: 现有的AI智能体难以直接处理复杂的单细胞转录组数据，且单细胞基础模型缺乏自然语言的可解释性。
method: 开发了ELISA框架，通过自动查询分类器整合scGPT嵌入、BioBERT语义匹配和LLM推理，实现对嵌入数据的直接分析与解释。
result: 在六个多样化数据集上，ELISA在细胞类型检索性能上显著超越CellWhisperer，并能以极高准确度复现已发表的生物学发现。
conclusion: ELISA成功弥合了转录组数据探索与生物学发现之间的鸿沟，为单细胞基因组学提供了一个强大且可解释的交互式分析工具。
---
