...
tags: ["query:med-ai"]
score: 8.0
evidence: 用于多模态临床预测的临床表示学习
tldr: 本研究针对临床机器学习模型在跨机构部署时面临的系统性分布偏移问题，提出了一种实践不变的表征学习框架。该框架通过将临床观察建模为潜在生理因素与环境相关过程的结合，利用对抗性正则化和不变风险惩罚来抑制环境特定伪影，从而解耦生理信号。实验表明，该方法在多项电子健康档案预测任务中显著提升了分布外性能和模型校准度，证明了在医疗人工智能中结合结构不变性与架构规模的重要性。
selection_source: fresh_fetch
motivation: 临床机器学习模型在部署时常面临因不同机构诊疗习惯和记录方式差异导致的系统性分布偏移，导致生理信号与环境伪影发生表征纠缠。
method: 提出一种实践不变表征学习框架，结合监督风险最小化、对抗性环境正则化和跨医院的不变风险惩罚，以学习鲁棒的临床表征。
result: 在多项跨机构电子健康档案预测任务中，该方法将分布外 AUROC 提升了 2 至 3 个百分点，同时保持了分布内性能并改善了模型校准。
conclusion: 显式处理系统性分布偏移并追求结构不变性，对于构建更具鲁棒性和可迁移性的医疗 AI 模型至关重要。
---
