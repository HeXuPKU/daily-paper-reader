...
title_zh: 生物基础模型学习到了什么样的拓扑与几何结构？来自 141 个假设的证据
...
tldr: 本研究通过AI驱动的自动化假设筛选框架，对scGPT和Geneformer等生物基础模型的内部表示进行了深入的几何与拓扑结构分析。通过测试141个假设，研究发现这些模型学习到了具有生物学意义的非平凡拓扑结构，且不同模型在全局表征上具有高度一致性，但在精确基因定位上存在差异。此外，这些结构在不同组织中表现出显著的局部化特征，为理解生物大模型的内在机制提供了重要证据。
selection_source: fresh_fetch
motivation: 旨在探究生物基础模型在处理单细胞基因表达时，其内部表示是否形成了具有生物学意义的几何与拓扑结构。
method: 采用AI驱动的执行器-头脑风暴循环，在52次迭代中对141个关于拓扑和几何结构的假设进行了大规模自动化筛选与验证。
result: 模型学习到了显著的非平凡拓扑结构，且不同模型间存在全局形状对齐，但信号在免疫组织中比在肺部组织中更具鲁棒性。
conclusion: 生物基础模型确实捕捉到了真实的生物几何结构，但这种结构在模型间共享的是全局特征而非精确位置，且具有明显的组织特异性。
---

## 摘要
当 scGPT 和 Geneformer 等生物基础模型处理单细胞基因表达时，其内部表示中形成了什么样的几何与拓扑结构？这种结构具有生物学意义还是训练伪影？我们对这些结论的置信度如何？我们通过自主的大规模假设筛选来回答这些问题：一个由 AI 驱动的“执行者-头脑风暴者”循环，在 52 次迭代中提出、测试并完善了 141 个几何与拓扑假设，涵盖了持久同调、流形距离、跨模型对齐、社区结构和有向拓扑，所有这些都带有明确的零假设控制和不相交的基因池划分。研究得出了三个主要发现：首先，模型学习到了真实的几何结构。基因嵌入邻域表现出非平凡的拓扑结构，在最弱的领域中，12 个 Transformer 层中有 11 层的持久同调在 p < 0.05 水平上显著，而在另外两个领域中则是 12 层全部显著。多级距离层次结构表明，在识别调控基因对方面，流形感知度量优于欧几里得距离，且图社区划分能够追踪已知的转录因子靶标关系。其次，这种结构在独立训练的模型之间是共享的。scGPT 和 Geneformer 之间的典型相关分析（CCA）对齐产生了 0.80 的典型相关系数和 72% 的基因检索准确率，但在测试的 19 种方法中，没有一种能可靠地恢复基因层面的对应关系。模型在基因空间的全局形状上达成了一致，但在精确的基因放置上则不然。第三，这种结构比最初看起来更具局部性。在应用于所有零假设族的严格零假设控制下，稳健的信号集中在免疫组织中，而肺部和外部肺部信号则大幅减弱。
