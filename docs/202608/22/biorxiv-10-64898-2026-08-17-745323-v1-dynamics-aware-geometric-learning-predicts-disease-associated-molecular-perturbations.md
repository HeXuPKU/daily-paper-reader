...
tags: ["query:gwas"]
score: 6.0
evidence: 用于遗传变异功能解读的深度学习方法，可与GWAS功能基因组整合相连
tldr: 错义突变和翻译后修饰是重塑蛋白功能的主要分子扰动，但传统上被独立研究，现有计算方法多依赖序列保守性或静态结构特征，缺乏对动态变化的理解。本文提出DynGeo-Pheno，一个集成蛋白语言模型与各向异性网络模型动力学的统一几何深度学习框架，以联合捕捉进化、结构与生物物理信息。在独立测试集上，它高精度预测疾病相关磷酸化位点和致病错义突变，并揭示动力学提供互补信息。结果发现致病扰动优先定位于配体结合口袋和蛋白互作界面，并呈现增强的长程耦合与机械稳定性，为可解释的病理机制预测建立了统一框架。
source: biorxiv
selection_source: fresh_fetch
motivation: 现有方法多依赖序列保守性或静态结构，忽略蛋白质动力学，且突变与修饰常被分开研究，缺乏统一框架。
method: 提出DynGeo-Pheno，融合蛋白语言模型与各向异性网络模型动力学，构建几何深度学习架构统一预测。
result: 独立测试集上高精度预测疾病相关磷酸化位点和致病突变，且发现动力学信息互补，致病残基富集于变构与机械稳定位点。
conclusion: 研究证明内在蛋白质动力学是致病性的重要互补决定因素，为可解释的AI病理预测和机制理解建立了统一框架。
---

## 摘要
错义突变和翻译后修饰（PTM）是重塑蛋白质功能的主要分子扰动，但传统上它们是分开研究的。当前的计算方法主要依赖于序列保守性或静态结构特征，限制了我们对于扰动如何改变蛋白质内在动力学的理解。我们提出了DynGeo-Pheno，一个统一的几何深度学习框架，将蛋白质语言模型表示与各向异性网络模型推导的动力学相结合，共同捕捉进化、结构和生物物理信息。DynGeo-Pheno在独立测试数据集上高精度地预测了疾病相关的磷酸化位点和致病性错义突变。消融分析表明，蛋白质动力学提供了超越序列进化和结构拓扑的互补信息，用于致病性预测。除了预测性能之外，DynGeo-Pheno还揭示了疾病相关的扰动优先定位于功能性结构区域，包括配体结合口袋和蛋白质-蛋白质相互作用界面。从机制上讲，磷酸化位点和错义突变似乎表现出不同但趋同的动力学特征。磷酸化位点优先出现在柔性调控区域，而致病性突变则富集于有序的结构元件中。尽管存在这些差异，两种扰动类型都显示出增强的长程耦合、增加的扰动响应性和提高的机械稳定性，表明致病残基优先占据机械约束和变构调控位点。本研究提供了有力的证据，表明内在蛋白质动力学是致病性的重要互补决定因素，并建立了一个统一的框架，用于可解释的人工智能预测以及对遗传和调控扰动如何塑造蛋白质功能的机制性理解。
