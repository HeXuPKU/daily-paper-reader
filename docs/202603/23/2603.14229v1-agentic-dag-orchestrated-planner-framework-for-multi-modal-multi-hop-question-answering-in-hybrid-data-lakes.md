...
title_zh: 面向混合数据湖中多模态、多跳问答的智能体化 DAG 编排规划器框架
...
## 摘要
企业日益需要针对结合了结构化表和非结构化文档的混合数据湖进行自然语言（NL）问答。当前部署的解决方案（包括基于 RAG 的系统）通常依赖于从每个存储库进行暴力检索并进行事后合并。这种方法效率低下且存在信息遗漏，更关键的是，它们缺乏对多跳推理的显式支持，即查询被分解为连续的步骤（跳），这些步骤可能在结构化和非结构化源之间来回穿梭。我们提出了 Agentic DAG-Orchestrated Transformer (A.DOT) Planner，这是一个用于多模态、多跳问答的框架，它将用户自然语言查询编译为跨越结构化和非结构化存储的有向无环图（DAG）执行计划。该系统将查询分解为可并行的子查询，结合了模式感知（schema-aware）推理，并在执行前应用结构和语义验证。执行引擎遵循生成的 DAG 计划，协调跨异构源的并发检索，将中间输出路由到依赖的子查询，并严格按照计划的逻辑依赖关系合并最终结果。先进的缓存机制结合了释义感知（paraphrase-aware）的模板匹配，使系统能够检测等效查询并重用先前的 DAG 执行计划以实现快速重新执行，而 DataOps 系统则处理验证反馈或执行错误。所提出的框架不仅提高了准确性和延迟表现，还产生了显式的证据链，从而能够验证检索内容、追踪数据血缘，并增强用户对系统输出的信任。在基准数据集上，A.DOT 在正确性上比基准模型实现了 14.8% 的绝对提升，在完整性上提升了 10.7%。
