...
## 摘要
从病理图像中估计切片级和斑块级（patch-level）基因表达谱，能够实现快速且低成本的分子分析，具有广泛的临床影响。尽管已取得显著成果，但现有方法仅将基因表达视为切片级或位点级信号，未考虑到测得的表达实际上源于底层细胞级表达的聚合。为了显式引入这种缺失的细胞分辨率引导，我们提出了一种细胞类型原型引导的神经网络（CPNN），该网络利用了公开可用的单细胞 RNA 测序数据集。由于单细胞测量存在噪声且未与组织学图像配对，我们首先估计细胞类型原型——即反映稳定基因-基因共变模式的平均表达谱。随后，CPNN 直接从图像中学习细胞类型组成权重，并对原型与观测到的批量（bulk）或空间表达之间的关系进行建模，从而提供了一个具有生物学依据且结构正则化的预测框架。我们在三个切片级数据集和三个斑块级空间转录组数据集上对 CPNN 进行了评估。在所有设置下，CPNN 在 Spearman 相关性方面均取得了最高性能。此外，通过可视化推断出的组成权重，我们的框架为哪些细胞类型驱动了预测的表达提供了可解释的见解。代码已在 https://github.com/naivete5656/CPNN 公开。
