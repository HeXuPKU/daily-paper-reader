...
tldr: 本研究系统地探讨了如何利用大语言模型（LLM）的嵌入技术为表格预测模型引入世界知识。通过对256种管道配置（包括预处理、嵌入模型和下游模型）进行基准测试，研究揭示了设计有效嵌入管道的关键因素。结果表明，合理的管道设计能显著提升预测性能，并为研究者提供了关于模型选择、数据处理和下游模型匹配的最佳实践指导。
selection_source: fresh_fetch
motivation: 旨在解决目前缺乏将LLM世界知识有效融入表格数据预测模型的系统性设计指南这一问题。
method: 通过基准测试256种管道配置，系统评估了8种预处理策略、16个嵌入模型和2种下游模型对预测性能的影响。
result: 发现拼接嵌入通常优于替换原始列，且嵌入模型的大小比其在公开排行榜上的排名更能反映实际性能。
conclusion: 梯度提升决策树是处理嵌入数据的强力下游模型，本研究为构建高效的表格嵌入管道提供了明确的实践建议。
---

## 摘要
嵌入是将大语言模型（LLM）的世界知识融入数据驱动的机器学习模型的一种强大方式。然而，关于如何为表格预测设计有效的基于 LLM 的嵌入流水线，目前的证据还很有限。在这项工作中，我们系统地基准测试了 256 种流水线配置，涵盖了 8 种预处理策略、16 种嵌入模型和 2 种下游模型。我们的结果表明，引入 LLM 的先验知识是否能提高预测性能，在很大程度上取决于具体的流水线设计。通常情况下，拼接嵌入往往优于用嵌入替换原始列。较大的嵌入模型往往会产生更好的结果，而公开排行榜排名和模型流行度则是较差的性能指标。最后，梯度提升决策树往往是强大的下游模型。我们的发现为研究人员和从业者构建更有效的表格预测任务嵌入流水线提供了指导。
