...
title_zh: PARHAF：一个由人工撰写的法语虚构患者临床报告语料库
...
tldr: 针对隐私法规限制医疗数据共享导致法语临床NLP发展受阻的问题，本研究推出了PARHAF语料库。该库由104名医学专家根据流行病学指导，为虚构患者编写了7394份高质量临床报告，涵盖18个医学专业。PARHAF完全匿名且开源，为法语临床语言模型的训练和评估提供了宝贵的隐私安全资源，并建立了一套可复制的合成语料库构建方法。
selection_source: fresh_fetch
motivation: 由于严格的隐私法规限制了真实医疗记录的共享，法语临床自然语言处理系统的开发面临数据短缺的挑战。
method: 组织104名医学住院医师根据流行病学指导和预设场景，编写并同行评审了涵盖18个专业的虚构患者临床报告。
result: 成功构建了包含7394份临床报告、涉及5009个虚构病例的大型开源法语语料库，涵盖通用及肿瘤、感染等特定领域。
conclusion: PARHAF为法语临床语言模型提供了隐私安全的训练资源，并为构建可共享的合成临床语料库提供了可复制的标准化方法。
---

## 摘要
临床自然语言处理（NLP）系统的发展受到医疗记录敏感性的严重阻碍，严格的隐私法规（特别是在法国和更广泛的欧盟地区）限制了数据共享。为了填补这一空白，我们推出了 PARHAF，这是一个大型的法语临床文档开源语料库。PARHAF 包含由专家撰写的临床报告，描述了真实但完全虚构的患者病例，使其在设计上具有匿名性且可自由共享。该语料库是利用一套结构化协议开发的，该协议结合了临床医生的专业知识与法国国家健康数据系统（SNDS）的流行病学指导，确保了广泛的临床覆盖面。共有来自 18 个专业的 104 名住院医师按照预定义的临床场景和文档模板撰写并同行评审了这些报告。该语料库包含 7394 份临床报告，涵盖了广泛的内外科专业的 5009 例患者病例。它包括一个旨在模拟真实世界住院分布的通用组件，以及四个支持肿瘤学、传染病和诊断编码信息提取用例的专业子集。文档根据 CC-BY 开源许可发布，其中一部分暂时封存，以便在受控条件下进行未来的基准测试。PARHAF 为在完全保护隐私的环境下训练和评估法语临床语言模型提供了宝贵的资源，并为在其他语言和卫生系统中构建可共享的合成临床语料库建立了一套可复制的方法论。
