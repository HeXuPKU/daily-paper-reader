...
tldr: "本研究针对大语言模型在处理长上下文时因依赖隐式注意力机制而导致的性能下降问题，提出将长文本处理外部化为显式的可执行交互。通过让编程智能体利用文件系统组织文本并使用原生工具（如代码和终端命令）进行操作，该方法在长文本推理、检索增强生成及超大规模语料库问答等任务中表现卓越。实验显示，该方法比现有最先进技术平均提升了17.3%，证明了编程智能体在处理海量信息方面的巨大潜力。"
selection_source: fresh_fetch
motivation: 传统大模型依赖隐式注意力机制处理长上下文，导致其在处理大规模数据时效率低下且性能显著下降。
method: 引入编程智能体作为通用接口，通过文件系统管理文本并利用可执行代码和终端工具进行显式交互处理。
result: "在多项基准测试中，编程智能体的表现平均优于现有最先进水平17.3%，且能处理高达三万亿token的语料。"
conclusion: 将长文本处理委托给具备工具使用能力的编程智能体，是替代语义搜索或单纯扩大上下文窗口的有效新方向。
---
