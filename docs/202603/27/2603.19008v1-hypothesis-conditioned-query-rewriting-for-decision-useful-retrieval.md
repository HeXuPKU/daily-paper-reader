...
tldr: 针对检索增强生成（RAG）在多选项决策任务中往往只检索主题相关而非决策相关信息的问题，本文提出了一种无需训练的预检索框架HCQR。该方法通过生成初步假设并将其重写为三个针对性查询（支持假设、区分选项、验证线索），将检索目标从主题导向转为证据导向。实验证明，HCQR在医学问答任务上显著提升了准确率，有效辅助模型做出更精准的决策。
selection_source: fresh_fetch
motivation: 现有的RAG方法通常依赖单一查询，导致检索到的背景信息虽与主题相关，却不足以区分不同的候选答案。
method: 提出HCQR框架，通过初步假设生成三个针对性查询，分别用于寻找支持证据、区分竞争选项以及验证问题中的关键线索。
result: 在MedQA和MMLU-Med数据集上，HCQR相比基础RAG分别提升了5.9和3.6个百分点，优于重排序和过滤等基准方法。
conclusion: 将检索目标从主题相关性转向证据导向的假设验证，能显著提高大语言模型在复杂决策任务中的表现。
---
