...
title_zh: 针对高维数据的快速且可扩展的单元格级鲁棒集成算法
...
tldr: 针对高维数据中常见的单元格级污染问题，本文提出了一种快速且可扩展的单元格稳健集成算法（FSCRE）。该算法通过集成稳健协方差估计、稳健LARS变量选择和竞争性子模型框架，填补了现有集成学习在处理单元格污染方面的空白。FSCRE具有尺度不变性和排列等变性，在模拟实验和生物信息学应用中展现出优异的变量选择精度和预测性能，为高维稳健统计分析提供了高效的解决方案。
selection_source: fresh_fetch
motivation: 传统高维变量选择方法和现代集成架构在面对单元格级数据污染时表现不佳，缺乏有效的稳健性处理机制。
method: 提出FSCRE算法，通过三阶段框架实现数据清洗、稳健协方差构建，并结合稳健相关性LARS与交叉验证进行变量选择与集成。
result: 实验结果表明，FSCRE在各种污染场景下的变量选择精确度、召回率和预测准确性均显著优于现有主流方法。
conclusion: 该研究成功构建了一个连接单元格稳健估计与高性能集成学习的统一框架，为复杂高维数据的稳健分析提供了有力工具。
---

## 摘要
高维数据的分析在基因组学等领域无处不在，但由于单元格级污染（即单个单元格而非整行数据被损坏）的存在，分析往往变得复杂。这种污染对标准的变量选择技术提出了重大挑战。虽然近期的集成方法引入了划分预测变量空间以处理高共线性的确定性框架，但这些现代架构并非为处理单元格级污染而设计，从而留下了关键的方法论空白。为了填补这一空白，我们提出了快速且可扩展的单元格级鲁棒集成（FSCRE）算法，这是一种集成三个关键统计阶段的新型多阶段框架。首先，该算法通过推导清洗后的数据矩阵和可靠的单元格级鲁棒协方差结构，建立了一个鲁棒的基础。随后，通过竞争性集成进行变量选择：一种基于相关性的鲁棒最小角回归（LARS）算法公式为多个子模型提出候选变量，并由交叉验证准则决定其最终分配。尽管其架构复杂，但所提方法具有基本的理论性质，包括数据缩放不变性和预测变量置换等变性，这确立了其客观性。通过广泛的模拟和生物信息学应用，我们证明了 FSCRE 在各种污染场景下，在变量选择的精确率、召回率和预测准确性方面均具有优越性能。这项工作提供了一个将单元格级鲁棒估计与高性能集成学习相结合的统一框架，其实现代码已在 CRAN 上发布。
