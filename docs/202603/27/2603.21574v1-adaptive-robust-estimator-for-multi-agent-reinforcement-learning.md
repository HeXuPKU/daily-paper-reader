...
tldr: 针对多智能体协作推理中存在的交互模糊性、信用分配困难以及奖励信号噪声导致的训练不稳定问题，本文提出了一种鲁棒的多智能体强化学习框架。该框架包含DACR结构化推理流程（回答-评论-重写）以明确各智能体贡献，以及ARE自适应鲁棒估计器来优化策略。实验证明，该方法在数学推理和具身智能任务中表现优异，能有效抵御奖励噪声，提升训练稳定性。
selection_source: fresh_fetch
motivation: 多智能体协作推理面临交互模糊导致的信用分配难题，且重尾噪声奖励易引发策略优化不稳定。
method: 提出了由DACR三阶段推理流程（回答、评论、重写）和ARE自适应鲁棒估计器组成的强化学习框架。
result: 在数学推理和具身智能基准测试中，该方法在同构和异构设置下均优于基线，且对奖励噪声具有更强的鲁棒性。
conclusion: 该框架通过结构化交互和鲁棒估计，有效解决了多智能体强化学习中的信用分配与噪声干扰问题，确保了训练的稳定性。
---
