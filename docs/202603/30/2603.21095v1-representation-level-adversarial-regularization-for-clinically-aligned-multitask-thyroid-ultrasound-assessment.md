...
tags: ["query:med-ai"]
score: 8.0
evidence: 用于甲状腺超声评估和风险分类的深度学习
tldr: 本研究针对甲状腺超声评估中医生标注不一导致的监督不一致问题，提出一种临床引导的多任务学习框架，同步预测结节掩码与TI-RADS等级。通过引入表征级对抗正则化（RLAR），利用潜在空间的对抗方向作为几何探针来惩罚任务间的梯度冲突。实验表明，该方法在提升风险分层准确性的同时保持了分割质量，优于传统多任务学习基准。
source: arxiv
...
method: 提出一种临床引导的多任务框架，并引入表征级对抗正则化（RLAR）通过潜在空间的对抗方向来显式控制任务间的干扰。
result: 在公开TI-RADS数据集上，该方法在维持分割精度的同时，显著提升了风险分层的性能。
conclusion: RLAR通过几何探针调节任务敏感性，有效缓解了多任务学习中的冲突，为临床超声辅助诊断提供了更稳健的方案。
---
