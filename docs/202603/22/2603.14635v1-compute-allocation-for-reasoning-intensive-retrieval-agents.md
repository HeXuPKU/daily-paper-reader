...
motivation: 旨在解决长程智能体在处理推理密集型检索时，如何在高昂的推理成本下优化计算资源的分配问题。
method: 在BRIGHT基准上利用Gemini 2.5系列模型，通过调整模型规模、推理思考时间和重排序深度来评估查询扩展与重排序的性能。
result: 重排序阶段通过增强模型能力和增加候选深度获得了显著提升，而查询扩展在超过轻量级模型后收益微弱，且推理思考对两阶段均无明显帮助。
conclusion: 计算资源应集中投入到重排序阶段，而非在检索流水线的各个阶段进行均匀分配。
---
