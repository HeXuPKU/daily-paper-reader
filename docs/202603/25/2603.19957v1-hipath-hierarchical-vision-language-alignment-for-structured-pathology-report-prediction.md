...
tldr: 针对病理报告结构化和多粒度的特点，现有视觉语言模型往往将其简化为扁平标签或自由文本。本文提出HiPath框架，基于冻结的UNI2和Qwen3骨干网络，通过分层补丁聚合器（HiPA）、分层对比学习（HiCL）和槽位掩码诊断预测（Slot-MDP）三个轻量化模块，实现了对多图像输入的高效处理和结构化报告生成。在749K例真实病理数据上的实验表明，该模型在准确率和安全性上均优于现有基准，并展现出良好的跨医院泛化能力。
selection_source: fresh_fetch
motivation: 现有病理视觉语言模型无法有效处理病理报告中复杂的多粒度结构化信息，通常将其简化为非结构化文本。
method: 提出包含HiPA多图像编码、HiCL分层对比学习和Slot-MDP结构化诊断生成三个模块的轻量化框架。
result: "在大规模真实数据集上达到68.9%的严格准确率和97.3%的安全率，性能显著优于同类基准模型。"
conclusion: HiPath通过分层对齐和结构化预测，为病理报告的自动化生成提供了一种高效、安全且泛化性强的解决方案。
---

## 摘要
病理报告是结构化的、多粒度的文档，记录了一个或多个解剖部位的诊断结论、组织学分级和辅助检查结果；然而，现有的病理视觉-语言模型（VLM）通常将此类输出简化为扁平标签或自由文本。我们提出了 HiPath，这是一个基于冻结的 UNI2 和 Qwen3 骨干网络构建的轻量级 VLM 框架，它将结构化报告预测作为其主要的训练目标。三个总计 1500 万参数的可训练模块分别解决了该问题的不同方面：用于多图像视觉编码的分层补丁聚合器（HiPA）、通过最优传输实现跨模态对齐的分层对比学习（HiCL），以及用于结构化诊断生成的基于插槽的掩码诊断预测（Slot-MDP）。在来自三家医院的 74.9 万例真实世界中文病理病例上进行的训练表明，HiPath 实现了 68.9% 的严格准确率和 74.7% 的临床可接受准确率，安全性达 97.3%，在相同冻结骨干网络下优于所有基准模型。跨医院评估证实了其泛化性能，严格准确率仅下降 3.4 个百分点，同时保持了 97.1% 的安全性。
