...
tags: ["query:med-ai"]
score: 6.0
evidence: 用于SQL推理的大语言模型和AI智能体
tldr: ReViSQL是一个旨在实现人类水平Text-to-SQL性能的框架。研究发现现有模型无法达到人类水平的主要瓶颈在于训练数据质量而非架构复杂性。通过构建包含2.5k个经过专家验证的BIRD-Verified数据集，并结合可验证奖励的强化学习（RLVR）以及推理时缩放技术，ReViSQL在BIRD基准测试上首次超越了人类水平的执行准确率，同时显著降低了推理成本。
source: arxiv
selection_source: fresh_fetch
figures_json: "[{\"url\": \"assets/figures/arxiv/2603.20004v1/fig-001.webp\", \"caption\": \"Figure 1: ReViSQL achieves human-level accuracy on an\", \"page\": 1, \"index\": 1, \"width\": 506, \"height\": 284}, {\"url\": \"assets/figures/arxiv/2603.20004v1/fig-002.webp\", \"caption\": \"Figure 2: The end-to-end ReViSQL framework. ReViSQL achieves human parity Text-to-SQL through three core steps: (a)\", \"page\": 5, \"index\": 2, \"width\": 1056, \"height\": 379}]"
motivation: 现有的Text-to-SQL模型即便采用复杂的流水线架构，也因训练数据中存在大量错误而无法达到人类水平的准确度。
method: 通过专家纠错构建高质量数据集BIRD-Verified，并利用可验证奖励的强化学习（RLVR）及推理时执行对齐与多数投票技术提升模型性能。
result: "ReViSQL-235B在BIRD Mini-Dev上达到93.2%的准确率，超越了92.96%的人类水平，且轻量级版本以更低成本匹配了先前SOTA。"
conclusion: 提高训练数据质量并结合强化学习，比增加系统架构复杂度更能有效提升Text-to-SQL模型的推理能力。
---
