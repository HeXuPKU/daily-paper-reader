...
title_zh: SCTA：一种从单细胞RNA测序中稳定且可解释地发现靶基因的智能体框架
...
tldr: 从单细胞RNA测序数据中发现治疗靶基因面临分析选择不稳定和难以解释的问题。现有工作流和通用分析智能体常产生不稳定或难解释的靶点假设。为此提出SCTA，一种决策中心的多智能体框架，将靶点发现分解为与关键决策点对齐的专门智能体，并用结构化生物学证据约束下游推理。在遗传性慢性胰腺炎消融研究中，SCTA的全证据集成在独立运行中最稳定，恢复疾病相关机制，表明决策感知智能体编排可提高靶点发现的鲁棒性和实用性。
source: arxiv
...
motivation: 单细胞RNA测序靶点发现对分析选择敏感，现有方法不稳定且不可解释。
method: 提出SCTA决策中心框架，将发现任务分解为专门智能体，并用结构化生物学证据约束推理。
result: 在胰腺炎消融研究中，SCTA全证据集成实现最稳定靶点选择，恢复生物学相干的疾病机制。
conclusion: 决策感知智能体编排可提高单细胞靶点发现的鲁棒性、可解释性和实用价值。
---

## 摘要
从单细胞RNA测序（scRNA-seq）数据中识别治疗靶基因仍然是转化生物学中的一个基本挑战。与批量检测不同，scRNA-seq能够捕获异质性细胞状态和罕见亚群，但同样的异质性使得靶点发现对整个流程中的分析选择高度敏感，包括预处理、细胞群体选择、差异表达分析和下游生物学解释。因此，现有工作流和通用分析智能体通常会产生不稳定或难以解释的靶点假设，限制了其在疾病导向发现中的可靠性。我们提出了SCTA（单细胞靶点智能体），这是一种以决策为中心的智能体框架，用于从scRNA-seq数据中稳定且可解释地发现靶基因。SCTA不将分析视为单一通用推理任务，而是将靶点发现分解为与单细胞流程中关键决策点相对应的专门智能体，并用结构化生物学证据约束下游推理。在针对遗传性慢性胰腺炎的代表性消融研究中，我们证明，在测试配置中，SCTA的完整证据整合在独立运行中产生了最稳定的靶点选择，同时恢复了先前研究中验证的生物学一致、疾病相关的机制。这些结果表明，针对单细胞分析结构定制的决策感知智能体编排可以提高精准医学中靶点发现的稳健性、可解释性和实用性。
