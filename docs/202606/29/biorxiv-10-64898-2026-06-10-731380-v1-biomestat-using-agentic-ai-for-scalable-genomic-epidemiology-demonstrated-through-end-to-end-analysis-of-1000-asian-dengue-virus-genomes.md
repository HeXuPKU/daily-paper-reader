...
title_zh: biomeStat：利用自主人工智能实现可扩展基因组流行病学——通过对1000个亚洲登革病毒基因组的端到端分析进行演示
authors: "Ariyaratne, D., Somaratna, N., Malavige, G. N."
date: 2026-06-23
pdf: "https://www.biorxiv.org/content/10.64898/2026.06.10.731380v1.full.pdf"
tags: ["query:med-ai"]
score: 7.0
evidence: 用于基因组流行病学的自主AI智能体，自动化生物信息学工作流
tldr: 基因组流行病学分析依赖专家手动调度多种工具和计算资源，效率低且易出错。biomeStat通过自主AI代理自动编写代码执行现有生物信息学工具，动态分配CPU/GPU并保证可重复性。在1000个登革病毒基因组全自动分析中，24小时内完成系统发育、贝叶斯动态、选择压力和结构映射，发现有效再生数约1.0、1869个免疫逃逸位点与表位共定位、176个保守药物靶点。该平台将自然语言意图转化为确定性计算，显著降低专家工作量。
source: biorxiv
selection_source: fresh_fetch
motivation: 现有基因组流行病学工作流程依赖专家手动调度工具和计算资源，导致效率低下和结果不可重复。
method: 开发biomeStat自主AI代理，通过自然语言指令自动编写代码执行标准生物信息学工具，动态分配CPU/GPU资源并确保可重复性。
result: 对1000个亚洲登革病毒基因组进行全自动分析，24小时内完成系统发育、贝叶斯动态、选择压力和结构分析，发现有效再生数约1.0、1869个免疫逃逸位点与B/T细胞表位共定位，176个抗病毒药物靶点绝对保守。
conclusion: biomeStat将自然语言意图与确定性计算桥接，将数周专家工作简化为单次透明分析，提升基因组流行病学可及性和效率。
---

## 摘要
基因组流行病学工作流程通常需要专家对多种专业工具进行整理、大量手动参数调整以及访问异构计算基础设施。虽然标准生成式AI模型在复杂的生物领域中常出现幻觉，但我们引入了biomeStat：一个自主AI代理，它作为严格的确定性协调器。通过在沙盒环境中自动编写代码以执行成熟的生物信息学工具，biomeStat动态配置计算资源（CPU和GPU）并保证可重复性，使其无需命令行专业知识即可立即为科学家所用。

为演示该平台，我们对2000年至2025年间从16个亚洲国家采样的1000个登革病毒（DENV）基因组进行了完全自主的基因组流行病学和结构分析。该代理无缝协调了系统发育重建（IQ-TREE、TreeTime）、贝叶斯系统动力学（通过NVIDIA H200 GPU的BEAST2）、选择压力分析（HyPhy）和结构映射（PyMOL）。该分析在不到24小时的挂钟时间内完成，揭示了地方性稳定（R_e ≈ 1.0），并识别了1869个候选免疫逃逸位点，这些位点在结构上与B细胞和T细胞表位共定位。此外，该代理验证了跨病毒复制复合体的176个高度保守的药物靶点残基，确认新出现的抗病毒药物JNJ-1802和NITD-688的耐药相关位点在所有四种血清型中绝对保守。通过在自然语言意图与确定性计算执行之间架起桥梁，biomeStat将数周的专家工作缩减为单次分析，并具有完整的方法透明度。
